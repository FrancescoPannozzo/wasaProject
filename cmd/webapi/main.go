...
func run() error {
	...
	_, errExec := dbconn.Exec("PRAGMA foreign_keys = ON;")
	if errExec != nil {
		logger.WithError(errExec).Error("error setting PRAGMA foreign_keys ON in DB")
		return fmt.Errorf("creating AppDatabase: %w", errExec)
	}
